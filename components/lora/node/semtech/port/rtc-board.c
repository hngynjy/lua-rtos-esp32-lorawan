...
#define LORA_TIMER_HZ  100000
#define LORA_TIMER_ADJ 5
#define LORA_TICK_USEC 10

static timg_dev_t *lora_timerg;        // Timer group
static int lora_timeri;                // Timer unit into timer group
...
static void IRAM_ATTR timer_isr(void *arg) {
    ...
    uint32_t intr_status = lora_timerg->int_st_timers.val;

    if((intr_status & BIT(timer_idx)) && timer_idx == lora_timeri) {
    	usecs += LORA_TICK_USEC;
    	if (usecs == 1000) {
    		msecs++;
    		usecs = 0;
    	...
    	lora_timerg->hw_timer[timer_idx].update = 1;
    	lora_timerg->int_clr_timers.t1 = 1;
    	lora_timerg->hw_timer[timer_idx].config.alarm_en = 1;
    }
}
...
void RtcInit( void )
{
	...
	timer_set_alarm_value(timer_group, timer_idx,  ((TIMER_BASE_CLK / LORA_TIMER_HZ) / 2) - LORA_TIMER_ADJ);
